...
export const TasksList = (props) => {
  const { currentTasks, handleEditTask, handleDeleteTask, handleToggleTaskStatus, viewType = "grid" } = props || {};
  return (
    <section aria-label="Tasks list" className="w-full">
      ...
      >
        {currentTasks?.map((task) => (
          <li key={task?.taskId} className={`w-full transition-all duration-300 list-none ${viewType === "grid" ? "scale-[0.97]" : "scale-100"}`}>
            <Task
              {...task}
